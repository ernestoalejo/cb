...
func validator(c *config.Config, q *registry.Queue) error {
  ...
  if !strings.HasSuffix(name, "Validator") {
    name += "Validator"
  }
...
class %s {
  ...
  public static function validateJson() {
    return validate(Input::json()->all());
  }

  public static funcion error($data, $msg) {
    Log::error($msg);
  ...
  public static function validate($data) {
    ...
    if (!is_array($data)) {
      return error($data, 'root is not an array');
    }
...
func generateFields(e *emitter, varname, result string, fields []*Field) error {
  ...
  for _, f := range fields {
    ...
    e.emitf(`  return error($data, 'key "%s" not set in data');`, f.Key);
    e.emitf(`}`)
    ...
    case "String":
      ...
      e.emitf(`  return error($data, 'key "%s" is not a string');`, f.Key);
      e.emitf(`}`)

    case "Integer":
      e.emitf(`if (is_string($value) {`)
      e.emitf(`  if (!ctype_digit($value) {`)
      e.emitf(`    return error($data, 'key "%s" is not a valid int');`, f.Key);
      e.emitf(`  }`)
      e.emitf(`  $value = intval($value);`)
      e.emitf(`}`)
      e.emitf(`if (!is_int($value)) {`)
      e.emitf(`  return error($data, 'key "%s" is not an int');`, f.Key);
      e.emitf(`}`)

    case "Boolean":
      e.emitf(`if (!is_bool($value)) {`)
      e.emitf(`  return error($data, 'key "%s" is not a boolean');`, f.Key);
      e.emitf(`}`)
...
func generateValidators(e *emitter, f *Field) error {
  for _, v := range f.Validators {
    ...
    case "MinLength":
      ...
      e.emitf(`  return error($data, 'key "%s" breaks the minlength validation');`, f.Key);
      e.emitf(`}`)

    case "Email":
      e.emitf(`if (filter_var($value, FILTER_VALIDATE_EMAIL) === false) {`)
      e.emitf(`  return error($data, 'key "%s" breaks the email validation');`, f.Key);
      e.emitf(`}`)
    ...
    case "DbPresent":
      ...
      e.emitf(`  return error($data, 'key "%s" breaks the dbpresent validation');`, f.Key);
      e.emitf(`}`)
    ...
    case "In":
      ...
      e.emitf(`  return error($data, 'key "%s" breaks the in validation');`, f.Key);
      e.emitf(`}`)

    case "Date":
      e.emitf(`$str = explode('/', $value);`)
      e.emitf(`if (count($str) !== 3 || !checkdate($str[1], $str[0], $str[2])) {`)
      e.emitf(`  return error($data, 'key "%s" breaks the date validation');`, f.Key);
      e.emitf(`}`)
    ...
    case "Before":
      ...
      e.emitf(`  return error($data, 'key "%s" breaks the before validation');`, f.Key);
      e.emitf(`}`)
